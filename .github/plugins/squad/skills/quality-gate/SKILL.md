...
- **Coder** — after every compilable intermediate state, run the build. Test gate is deferred to Tester but coverage pre-check is OK.
- **Tester** — must pass build + test + coverage before `[DONE]`.
- **Reviewer** — runs this gate independently as part of clean-context review. A failure here is `[BLOCKED]` regardless of what Tester said.
- **Infra** — after any AppHost edit, confirm `dotnet build --no-incremental -warnaserror` is green before `[CHECKPOINT]` or `[DONE]`.
